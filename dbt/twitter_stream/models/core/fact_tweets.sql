...
select
    ...
    stg_raw_tweets.lang as language,
...
join {{ ref('dim_user') }} as dim_user on raw_tweets.user.id = dim_user.user_id
join "{{ source('seed', 'lang_names.csv') }}" as lang_names on lang_names.lang_code = stg_raw_tweets.lang